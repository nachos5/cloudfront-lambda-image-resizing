...
docker run --rm --volume ${PWD}/dist/origin-response:/build ${DOCKER_IMAGE} /bin/bash -c "source ~/.bashrc; npm init -f -y; npm install sharp --save; npm install querystring --save; npm install --only=prod"
